...
function parseWithTemplate (data) {
  ...
  str = replace(str, 'vars', varList.map(function (v) {
    ...
    if(v.type.indexOf('Vector') > -1){
      ...
      deps += 'var ' + v.type + ' = require(\'' + depPath + messageCvt.formatFileName(v.type) + '\').class;';
    }
    return str + ';';
  }).join(''));
  ...
  if(result){
    ...
    str = str.replace('d2com.NetworkMessage', 'require(\'' + dir + messageCvt.formatFileName(abstractClassName) + '\')');
  }
