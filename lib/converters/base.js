...
module.exports = {
  ...
  writeFile: function(filename, outputFolder){
    ...
    var asClass = adReader(filename);
    console.log(asClass.class, asClass.super);
...
function parseFunctionBody (data) {
  return data[0] === '}' ? '' : data.replace(/ as \w+/g, '').replace(/:(uint|int);/g, ' = 0;').replace(/:[\w|*]+/g, '').replace(/super\./g, 'this.');
}