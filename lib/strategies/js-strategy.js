...
module.exports = {
  ...
  typeConverter: function (asClass) {
    ...
    return template('./lib/templates/type.tpl', [
      ...
      {key: 'vars', value: asClass.properties.map(parseVariable).join('')},
      {key: 'serialize', value: _.findWhere(asClass.functions, {name: 'serializeAs_' + asClass.class}).body},
      {key: 'deserialize', value: _.findWhere(asClass.functions, {name: 'deserializeAs_' + asClass.class}).body}
    ]).replace('\n\n\n', '\n').replace(/super.deserialize/g, 'this.deserializeAs_' + asClass.super).replace(/super.serialize/g, 'this.serializeAs_' + asClass.super);
  },
...
function formatSuperImport (asClass) {
  ...
  return 'require(\'util\').inherits(' + asClass.class + ', ' + 'require(\''+ depPath + '\').class);';
}
