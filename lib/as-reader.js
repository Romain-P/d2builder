...
function getConstants (data) {
  var loc = data.match(/private static const (.)+;/g) || [];
  var loc1 = data.match(/public static const (.)+;/g) || [];
  var privateConstants = loc.map(function (v) {
    v = v.replace('private static const ', '');
    v = parseVariable(v);
    v.visivility = 'private';
    return v;
  });
  var publicConstants = loc1.map(function (v) {
    v = v.replace('public static const ', '');
    v = parseVariable(v);
    v.visivility = 'public';
    return v;
  });
  return publicConstants.concat(privateConstants);
}