...
var AsReader = function (filename) {
  ...
  asClass.imports = toDict(getImports(data), 'class');
  var ns;
  if((ns = getNamespace(data)))
  {
    asClass.namespace = ns;
  }
  asClass.properties = toDict(getProperties(data), 'name');
  asClass.constants = toDict(getConstants(data), 'name');
  asClass.functions = toDict(getFunctions(data), 'name');
  return asClass;
};
